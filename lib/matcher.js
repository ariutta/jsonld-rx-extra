...
var JsonldMatcher = function(jsonldRx) {
  ...
  function addCharacteristicValuesProperty(expanded, expandedKeys, matcher) {
    ...
    var stringCharacteristicKeys = matcher.stringCharacteristicKeys;
    var functionCharacteristicKeys = matcher.functionCharacteristicKeys;
    var characteristicValues = [];
    if (!_.isEmpty(functionCharacteristicKeys)) {
      characteristicValues = characteristicValues.concat(
        functionCharacteristicKeys.map(function(fn) {
          return fn(expanded);
        })
    ...
    var matchingCharacteristicKeys = _.intersection(expandedKeys, stringCharacteristicKeys);
    if (matchingCharacteristicKeys) {
      characteristicValues = characteristicValues.concat(
        matchingCharacteristicKeys.reduce(function(accumulator, stringCharacteristicKey) {
          var currentResults = expanded[stringCharacteristicKey];
  ...
  function enhanceMatchers(matchers) {
    ...
    if (!matchers) {
      matchers = sameAsStringSets.map(function(sameAsStringSet) {
        return {
          ...
          stringCharacteristicKeys: sameAsStringSet
        };
      });
    } else {
      matchers = matchers.map(function(matcher) {
        ...
        matcher.functionCharacteristicKeys = characteristicKeys.filter(_.isFunction);

        var stringCharacteristicKeys = characteristicKeys.filter(_.isString);

        var moreIdentifierStrings = sameAsStringSets.find(function(sameAsStringSet) {
          return _.intersection(sameAsStringSet, stringCharacteristicKeys).length > 0;
        });

        if (!_.isEmpty(moreIdentifierStrings)) {
          stringCharacteristicKeys = _.union(stringCharacteristicKeys, moreIdentifierStrings);
        }

        matcher.stringCharacteristicKeys = stringCharacteristicKeys;
    ...
    .map(function(matcher) {
      matcher.characteristicValuesPropertyKey = CHARACTERISTIC_VALUES_PROPERTY_KEY_STUB +
          matcher.stringCharacteristicKeys[0];
      return matcher;
    })
    .map(function(matcher) {
      matcher.stringCharacteristicKeys = matcher.stringCharacteristicKeys
      .map(function(stringCharacteristicKey) {
        // TODO should we be turning all stringCharacteristicKeys into values of type @id?
        // if the user specifies a non @id stringCharacteristicKey, we turn it into an @id.
        // TODO is this regular expression appropriate?
        // see also
        // http://stackoverflow.com/questions/161738/
        // what-is-the-best-regular-expression-to-check-if-a-string-is-a-valid-url
        var re = new RegExp(/\/\//);
        if (re.test(stringCharacteristicKey) || stringCharacteristicKey.indexOf('@id') === 0) {
          return stringCharacteristicKey;
        } else {
          return PLACEHOLDER_VOCAB_IRI + stringCharacteristicKey;
        }
      });
      return matcher;
    })
   ...
   * @param {string[]|function[]} matchers.characteristicKeys IRI string that indicates a
   *                              characteristic to use for matching, or
   *                              a function that returns such a string.
        ...
        if (referenceRecordItems.length === 1) {
          ...
          .flatMap(function(referenceRecordItem) {
            ...
            var matchingKeySet = _.map(matchers, function(matcher) {
              return matcher.stringCharacteristicKeys;
            })
            .find(function(stringCharacteristicKeys) {
              return _.intersection(stringCharacteristicKeys, referenceRecordItemKeys).length > 0;
            });
