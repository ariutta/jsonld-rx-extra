  ...
  function getTotalWeight(matchers, processedToMatchRecord, referenceRecord) {
    return Rx.Observable.from(matchers).map(function(matcher) {
      var characteristicValueSets = getCharacteristicValueSets(
          matcher, processedToMatchRecord, referenceRecord);
...
//mValueOrGenerator(processedToMatchRecord, toMatchValues, referenceRecord, referenceValues) :
          mValueOrGenerator(matcher, processedToMatchRecord, referenceRecord) :
          mValueOrGenerator;

      var uValueOrGenerator = matcher.probabilityFalsePositive;
      var probabilityFalsePositive = _.isFunction(uValueOrGenerator) ?
          uValueOrGenerator(matcher, processedToMatchRecord, referenceRecord) :
          uValueOrGenerator;
